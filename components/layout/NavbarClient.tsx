...
export default function NavbarClient({ initialSettings = {} }: { initialSettings?: Record<string,any> }) {
  ...
  const cfg: Record<string,any> = { ...DEFS, ...initialSettings }
  ...
  const navH = (parseInt(cfg.navbarHeight) || 52) + 'px'
  const isCenter = cfg.searchPosition === 'center'
  const isRight = cfg.searchPosition === 'right'
  ...
  return (
    <>
      ...
      <nav style={{ background: cfg.navBg||'#1a1f3c', padding:'0 16px', display:'flex', alignItems:'center', gap:'10px', height:navH, position:'sticky', top:0, zIndex:100, boxShadow:'0 2px 8px rgba(0,0,0,.3)' }}>
        ...
        {cfg.showSearchBar && (!isCenter && !isRight) && (
        ...
        {cfg.showSearchBar && (isCenter || isRight) && <div style={{ flex:1 }} />}
        {cfg.showSearchBar && (isCenter || isRight) && (
          <form onSubmit={doSearch} style={{ position:'relative', width:(cfg.searchWidth==='100%'?'100%':cfg.searchWidth+'px'), flexShrink:0 }}>
            <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search games..."
              style={{ width:'100%', background:'rgba(255,255,255,.1)', border:'1px solid rgba(255,255,255,.2)', borderRadius:'8px', height:'36px', padding:'0 36px 0 12px', color:'#fff', fontSize:'13px', outline:'none', fontFamily:'inherit', boxSizing:'border-box' }} />
            <button type="submit" style={{ position:'absolute', right:'8px', top:'50%', transform:'translateY(-50%)', background:'none', border:'none', cursor:'pointer', color:'rgba(255,255,255,.6)', fontSize:'16px', padding:0 }}>\u2315</button>
          </form>
        )}
        {(isCenter || isRight) && <div style={{ flex:1 }} />}
        {cfg.showHotButton && (
