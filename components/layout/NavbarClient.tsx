...
export default function NavbarClient({ initialSettings = {} }: { initialSettings?: Record<string,any> }) {
  ...
  useEffect(() => {
    // Re-fetch on mount to ensure fresh data
  ...
  const navH = parseInt(cfg.navbarHeight) || 52

  return (
    <>
      {cfg.showAnnouncement && (
        <div style={{ background: cfg.announcementBg || '#4f46e5', color: '#fff', textAlign: 'center', padding: '6px 12px', fontSize: '13px', fontWeight: 500, lineHeight: '1.4' }}>
          {cfg.announcementText}
        </div>
      )}

      <nav style={{ background: cfg.navBg || '#1a1f3c', padding: '0 16px', display: 'flex', alignItems: 'center', gap: '8px', height: navH + 'px', position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 2px 8px rgba(0,0,0,.3)' }}>
        ...
        {cfg.showSearchBar && (
          <form onSubmit={doSearch} style={{ flex: 1, position: 'relative', minWidth: 0, maxWidth: cfg.searchWidth === '100%' ? '100%' : cfg.searchWidth + 'px' }}>
            <input
              value={q}
            ...
            />
            <button type="submit" style={{ position: 'absolute', right: '8px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,.6)', fontSize: '16px', padding: 0, lineHeight: 1 }}>⌕</button>
          </form>
        )}
      ...
      {cfg.showCatsBar && (
        <div style={{ background: '#1e2440', borderBottom: '1px solid rgba(255,255,255,.05)' }}>
          <style>{`.cgpc-scrollbar::-webkit-scrollbar{display:none}`}</style>
          <div className="cgpc-scrollbar" style={{ display: 'flex', overflowX: 'auto' as any, scrollbarWidth: 'none' as any, padding: '0 8px' }}>
            {CATS.map(cat => (
              <Link
                key={cat}
                href={cat === 'All' ? '/games' : '/games?category=' + encodeURIComponent(cat)}
                ...
                style={{
                  ...
                  fontWeight: 500,
                  whiteSpace: 'nowrap',
                  color: activeCat === cat ? '#818cf8' : '#94a3b8',
                  textDecoration: 'none',
                  borderBottom: activeCat === cat ? '2px solid #818cf8' : '2px solid transparent',
                  flexShrink: 0,
                  transition: 'color .2s',
                }}>
                {cat}
