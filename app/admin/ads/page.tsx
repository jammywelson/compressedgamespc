...
export default function Page() {
  const [cfg, setCfg] = useState<Record<string,any>>({"adsEnabled":false,"popupEnabled":false,"adFrequency":5,"headerAd":"","footerAd":"","sidebarAd":"","inContentAd":"","popupAd":""})
  const [saving, setSaving] = useState(false)
  const [saved, setSaved] = useState(false)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetch('/api/settings?key=ads&t='+Date.now(), {cache:'no-store'}).then(r=>r.json()).then((d:any)=>{if(d)setCfg(p=>({...p,...d}))}).catch(()=>{}).finally(()=>setLoading(false))
  }, [])
  ...
  return (
    <div style={{maxWidth:'860px'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'24px',flexWrap:'wrap' as any,gap:'12px'}}>
        <h1 style={{margin:0,fontSize:'22px',fontWeight:700,color:'#0f172a'}}>\ud83d\udcb0 Ad Inserter</h1>
      ...
      <div style={card}>
        <h3 style={{margin:'0 0 16px',fontSize:'16px',fontWeight:700}}>Global Settings</h3>
        <div style={{display:'flex',gap:'10px',flexWrap:'wrap' as any,marginBottom:'14px'}}><T k='adsEnabled' l='Enable Ads' /><T k='popupEnabled' l='Popup Ads' /></div>
        <div><label style={lbl}>In-Content Frequency (every N games)</label><input style={{...inp,width:'120px'}} type='number' value={cfg.adFrequency||5} onChange={(e:any)=>set('adFrequency',e.target.value)} /></div>
      </div>
      {(['headerAd','footerAd','sidebarAd','inContentAd','popupAd'] as string[]).map((k:string) => (
        <div key={k} style={card}>
          <h3 style={{margin:'0 0 12px',fontSize:'15px',fontWeight:700,textTransform:'capitalize' as any}}>{k.replace('Ad',' Ad')}</h3>
          <textarea style={{...inp,height:'100px',fontFamily:'monospace',fontSize:'12px',resize:'vertical'}} value={cfg[k]||''} onChange={(e:any)=>set(k,e.target.value)} placeholder='Paste ad code (Google AdSense, etc.)...' />
        </div>
      ))}
