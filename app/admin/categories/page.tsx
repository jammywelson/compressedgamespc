...
const DEFAULT_IDS=DEFAULT_CATS.map(c=>c.id)
...
export default function CategoriesPage(){
  ...
  const save=async(updated:any[])=>{
    setCats(updated)
    await fetch('/api/settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({key:'categories',value:updated})})
  }
  ...
  const addCat=async()=>{
    ...
    const cat={id:slug,name:newCat.name.trim(),slug,icon:newCat.icon||'🎮',seoTitle:'',seoDesc:'',seoKeywords:''}
    await save([...cats,cat])
    setNewCat({name:'',slug:'',icon:'🎮'});setShowAdd(false);showMsg('Category add ho gayi!')
  }

  const saveEdit=async()=>{
    if(!editCat)return
    await save(cats.map(c=>c.id===editCat.id?editCat:c))
    setEditCat(null);showMsg('Category update ho gayi!')
  }
    ...
    <div style={{background:'#f0f2f8',minHeight:'100vh'}}>
      {delId&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.45)',zIndex:999,display:'flex',alignItems:'center',justifyContent:'center'}}>
          <div style={{background:'#fff',borderRadius:'12px',padding:'24px',width:'320px',textAlign:'center' as any}}>
            ...
            <div style={{fontSize:'16px',fontWeight:700,color:'#111827',marginBottom:'18px'}}>Delete "{cats.find(c=>c.id===delId)?.name}"?</div>
      ...
      {editCat&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.5)',zIndex:999,display:'flex',alignItems:'center',justifyContent:'center',padding:'20px'}}>
          <div style={{background:'#fff',borderRadius:'12px',width:'100%',maxWidth:'580px',maxHeight:'88vh',overflow:'auto'}}>
            <div style={{padding:'16px 20px',borderBottom:'1px solid #e5e7eb',display:'flex',justifyContent:'space-between',alignItems:'center',position:'sticky',top:0,background:'#fff'}}>
              <div style={{fontSize:'16px',fontWeight:700,color:'#111827'}}>Edit — {editCat.name}</div>
              ...
              <div style={{borderTop:'1px solid #f3f4f6',paddingTop:'14px'}}>
                ...
                <div style={{display:'grid',gap:'10px'}}>
                  <div>
                    ...
                    <input style={SI} value={editCat.seoTitle} onChange={e=>setEditCat({...editCat,seoTitle:e.target.value})} placeholder={`${editCat.name} PC Games Free Download`}/>
                    <div style={{fontSize:'10px',color:editCat.seoTitle?.length>60?'#e53935':'#9ca3af',marginTop:'2px'}}>{editCat.seoTitle?.length||0}/60</div>
                  </div>
                  <div>
                    <label style={LB}>Meta Description</label>
                    <textarea style={{...SI,minHeight:'70px',resize:'vertical' as any}} value={editCat.seoDesc} onChange={e=>setEditCat({...editCat,seoDesc:e.target.value})} placeholder={`Free ${editCat.name} PC games highly compressed download`}/>
                    <div style={{fontSize:'10px',color:editCat.seoDesc?.length>160?'#e53935':'#9ca3af',marginTop:'2px'}}>{editCat.seoDesc?.length||0}/160</div>
                  </div>
                  <div>
                    <label style={LB}>Keywords</label>
                    <input style={SI} value={editCat.seoKeywords} onChange={e=>setEditCat({...editCat,seoKeywords:e.target.value})}/>
                    <div style={{display:'flex',gap:'4px',flexWrap:'wrap' as any,marginTop:'6px'}}>
                      {[`${editCat.name.toLowerCase()} games download`,`${editCat.name.toLowerCase()} highly compressed`,`free ${editCat.name.toLowerCase()} pc`].map(kw=>(
      ...
      <div style={{background:'#fff',borderBottom:'1px solid #e5e7eb',padding:'0 24px',height:'54px',display:'flex',alignItems:'center',gap:'12px'}}>
        ...
        {msg&&<span style={{fontSize:'12px',color:'#16a34a',fontWeight:500}}>{msg}</span>}
      ...
      <div style={{padding:'24px'}}>
        {showAdd&&(
          <div style={{background:'#fff',border:'1px solid #c7d2fe',borderRadius:'10px',padding:'20px',marginBottom:'16px'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'80px 1fr auto',gap:'12px',alignItems:'flex-end'}}>
              <div><label style={LB}>Icon</label><input style={{...SI,textAlign:'center' as any,fontSize:'20px'}} value={newCat.icon} onChange={e=>setNewCat(n=>({...n,icon:e.target.value}))} placeholder="🎮"/></div>
              <div><label style={LB}>Name *</label><input style={SI} value={newCat.name} onChange={e=>setNewCat(n=>({...n,name:e.target.value}))} placeholder="e.g. RPG" autoFocus/></div>
              <button onClick={addCat} style={{background:'#4f46e5',color:'#fff',border:'none',borderRadius:'7px',padding:'9px 20px',fontSize:'13px',fontWeight:600,cursor:'pointer',fontFamily:'inherit',height:'38px'}}>Add</button>
            </div>
          </div>
        )}
        {loading?(
            ...
            {cats.map(cat=>(
              <div key={cat.id} style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:'10px',padding:'16px'}}>
                ...
                <div style={{display:'flex',gap:'6px'}}>
                  <button onClick={()=>setEditCat({...cat})} style={{flex:1,background:'#eef2ff',color:'#4f46e5',border:'1px solid #c7d2fe',borderRadius:'5px',padding:'5px',fontSize:'11px',cursor:'pointer',fontFamily:'inherit'}}>Edit + SEO</button>
                  <a href={`/games?category=${encodeURIComponent(cat.name)}`} target="_blank" style={{background:'#f3f4f6',color:'#374151',border:'1px solid #e5e7eb',borderRadius:'5px',padding:'5px 8px',fontSize:'11px',textDecoration:'none'}}>View</a>
