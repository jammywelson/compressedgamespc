...
export default function CategoriesPage(){
  ...
  const saveEdit=async()=>{
    ...
    setEditCat(null);showMsg('✓ Category update ho gayi!');setSaving(false)
  }
          ...
          <div style={{background:'#fff',borderRadius:'12px',width:'100%',maxWidth:'580px',maxHeight:'90vh',overflow:'auto'}}>
            ...
            <div style={{padding:'20px',display:'grid',gap:'14px'}}>
              <div style={{display:'grid',gridTemplateColumns:'70px 1fr 1fr',gap:'12px'}}>
                ...
                <div><label style={LB}>Name</label><input style={SI} value={editCat.name} onChange={e=>setEditCat({...editCat,name:e.target.value,slug:e.target.value.toLowerCase().replace(/\s+/g,'-').replace(/[^a-z0-9-]/g,'')})}/></div>
                <div><label style={LB}>Slug (URL)</label><input style={{...SI,color:'#0891b2'}} value={editCat.slug} onChange={e=>setEditCat({...editCat,slug:e.target.value.toLowerCase().replace(/[^a-z0-9-]/g,'')})} placeholder="category-slug"/></div>
              </div>
              <div style={{borderTop:'1px solid #f3f4f6',paddingTop:'14px'}}>
                ...
                <div style={{display:'grid',gap:'10px'}}>
                  <div>
                    ...
                    <input style={SI} value={editCat.seoTitle} onChange={e=>setEditCat({...editCat,seoTitle:e.target.value})} placeholder={`${editCat.name} PC Games Free Download Highly Compressed`}/>
                  ...
                  <div>
                    ...
                    <textarea style={{...SI,minHeight:'70px',resize:'vertical' as any}} value={editCat.seoDesc} onChange={e=>setEditCat({...editCat,seoDesc:e.target.value})} placeholder={`Download free highly compressed ${editCat.name} PC games. Direct links.`}/>
                  ...
                  <div>
                    ...
                    <input style={SI} value={editCat.seoKeywords} onChange={e=>setEditCat({...editCat,seoKeywords:e.target.value})} placeholder={`${editCat.name.toLowerCase()} games, ${editCat.name.toLowerCase()} highly compressed`}/>
                    <div style={{display:'flex',gap:'4px',flexWrap:'wrap' as any,marginTop:'6px'}}>
                      {[`${editCat.name.toLowerCase()} games download`,`${editCat.name.toLowerCase()} highly compressed`,`free ${editCat.name.toLowerCase()} pc games`].map(kw=>(
        ...
        {showAdd&&(
          <div style={{background:'#fff',border:'1px solid #c7d2fe',borderRadius:'10px',padding:'20px',marginBottom:'16px'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'70px 1fr 1fr auto',gap:'12px',alignItems:'flex-end'}}>
              ...
              <div><label style={LB}>Name *</label><input style={SI} value={newCat.name} onChange={e=>setNewCat(n=>({...n,name:e.target.value,slug:e.target.value.toLowerCase().replace(/\s+/g,'-').replace(/[^a-z0-9-]/g,'')}))} placeholder="e.g. RPG" autoFocus/></div>
              <div><label style={LB}>Slug</label><input style={{...SI,color:'#0891b2'}} value={newCat.slug} onChange={e=>setNewCat(n=>({...n,slug:e.target.value.toLowerCase().replace(/[^a-z0-9-]/g,'')}))} placeholder="rpg"/></div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(210px,1fr))',gap:'12px'}}>
            {cats.map(cat=>(
              <div key={cat.id} style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:'10px',padding:'16px'}}>
                ...
                {cat.seoTitle&&<div style={{fontSize:'10px',color:'#16a34a',marginBottom:'8px',background:'#f0fdf4',padding:'3px 6px',borderRadius:'4px',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' as any}}>✓ {cat.seoTitle}</div>}
